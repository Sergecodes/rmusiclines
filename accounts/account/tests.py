...
import accounts.app_settings
from accounts.account.forms import BaseSignupForm, ResetPasswordForm, SignupForm
from accounts.account.models import (
    EmailAddress,
    EmailConfirmation,
    EmailConfirmationHMAC,
)
from accounts.tests import Mock, TestCase, patch
from accounts.utils import get_user_model, get_username_max_length
...
@override_settings(
    ...
    ACCOUNT_ADAPTER="accounts.account.adapter.DefaultAccountAdapter",
    ACCOUNT_USERNAME_REQUIRED=True,
)
class AccountTests(TestCase):
    def setUp(self):
        if "accounts.socialaccount" in settings.INSTALLED_APPS:
            ...
            if accounts.app_settings.SITES_ENABLED:
    ...
    @override_settings(
        ACCOUNT_USERNAME_VALIDATORS="accounts.account.tests.test_username_validators"
    )
...
class UtilsTests(TestCase):
    ...
    def test_url_str_to_pk_identifies_UUID_as_stringlike(self):
        with patch("accounts.account.utils.get_user_model") as mocked_gum:
...
class ConfirmationViewTests(TestCase):
    ...
    @patch("accounts.account.views.perform_login")
    @patch("accounts.account.utils.get_user_model", return_value=UUIDUser)
    def test_login_on_confirm_uuid_user(self, mocked_gum, mock_perform_login):
        ...
        with patch("accounts.account.views.EmailConfirmationHMAC", confirmation):
