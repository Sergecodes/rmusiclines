...
class AuthenticationBackend(ModelBackend):
    ...
    def _authenticate_by_email(self, **credentials):
        # Even though accounts will pass along `email`, other apps may
    ...
    def _stash_user(cls, user):
        ...
        checks `is_active`. Now, earlier versions of accounts did not do this
        and simply returned the user as authenticated, even in case of
        `is_active=False`. For accounts scope, this does not pose a problem, as
        these users are properly redirected to an account inactive page.

        This does pose a problem when the accounts backend is used in a
        different context where accounts is not responsible for the login. Then,
        ...
        In order to preserve the accounts behavior while respecting Django's
        logic, we stash a user for which the password check succeeded but
        `user_can_authenticate()` failed. In the accounts authentication logic,
